...
export function Achievement({ achievement, description, icon }: AchievementProps) {
  const unlocked = achievement.achieved ?? false;
  ...
  return (
    <div className={`card-clean p-4 flex items-center gap-4 ${!unlocked ? 'opacity-50' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium truncate">{achievement.name ?? 'Unnamed Achievement'}</p>
